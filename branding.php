...
try {
    $sql = "CREATE TABLE IF NOT EXISTS branding (
        ...
        logo_url_primary VARCHAR(255),
        logo_url_favicon VARCHAR(255),
        logo_url_social_share VARCHAR(255),
        color_background VARCHAR(7),
        color_primary_text VARCHAR(7),
        color_secondary_text VARCHAR(7),
        color_button VARCHAR(7),
        color_button_text VARCHAR(7),
    ...
    // Check if there's any branding data
    $stmt = $conn->query("SELECT COUNT(*) FROM branding");
    $count = $stmt->fetchColumn();

    // If no branding data exists and branding.json exists, import it
    if ($count == 0 && file_exists('branding.json')) {
        $jsonData = json_decode(file_get_contents('branding.json'), true);
        
        $stmt = $conn->prepare("INSERT INTO branding (
            company_name, tagline, founded_year, headquarters_location, website_url,
            logo_url_primary, logo_url_favicon, logo_url_social_share,
            color_background, color_primary_text, color_secondary_text, color_button, color_button_text,
        ...
        $stmt->execute([
            $jsonData['companyInfo']['name'] ?? null,
            $jsonData['companyInfo']['tagline'] ?? null,
            $jsonData['companyInfo']['foundedYear'] ?? null,
            $jsonData['companyInfo']['headquartersLocation'] ?? null,
            $jsonData['companyInfo']['websiteUrl'] ?? null,
            $jsonData['visualIdentity']['logoUrl']['primary'] ?? null,
            $jsonData['visualIdentity']['logoUrl']['favicon'] ?? null,
            $jsonData['visualIdentity']['logoUrl']['socialShare'] ?? null,
            $jsonData['visualIdentity']['colors']['background'] ?? '#ffffff',
            $jsonData['visualIdentity']['colors']['primaryText'] ?? '#333333',
            $jsonData['visualIdentity']['colors']['secondaryText'] ?? '#666666',
            $jsonData['visualIdentity']['colors']['button'] ?? '#e0e0e0',
            $jsonData['visualIdentity']['colors']['buttonText'] ?? '#333333',
            $jsonData['socialMedia']['facebook'] ?? null,
            $jsonData['socialMedia']['twitter'] ?? null,
            $jsonData['socialMedia']['instagram'] ?? null,
            $jsonData['socialMedia']['linkedin'] ?? null,
            $jsonData['socialMedia']['youtube'] ?? null,
            $jsonData['contactInformation']['email'] ?? null,
            $jsonData['contactInformation']['phone'] ?? null,
            $jsonData['contactInformation']['address'] ?? null
        ]);

        // Create backup of the JSON file
        if (file_exists('branding.json')) {
            copy('branding.json', 'branding.backup.json');
            unlink('branding.json'); // Remove the original JSON file
        }
    }
} catch(PDOException $e) {
    error_log("Failed to create branding table: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        try {
            // Update branding data in database
            $stmt = $conn->prepare("UPDATE branding SET
                company_name = ?,
                tagline = ?,
                founded_year = ?,
                ...
                contact_phone = ?,
                contact_address = ?
                WHERE id = (SELECT id FROM (SELECT id FROM branding LIMIT 1) tmp)");

            $stmt->execute([
                $data['companyInfo']['name'] ?? null,
                $data['companyInfo']['tagline'] ?? null,
                $data['companyInfo']['foundedYear'] ?? null,
                $data['companyInfo']['headquartersLocation'] ?? null,
                $data['companyInfo']['websiteUrl'] ?? null,
                $data['visualIdentity']['logoUrl']['primary'] ?? null,
                $data['visualIdentity']['logoUrl']['favicon'] ?? null,
                $data['visualIdentity']['logoUrl']['socialShare'] ?? null,
                $data['visualIdentity']['colors']['background'] ?? '#ffffff',
                $data['visualIdentity']['colors']['primaryText'] ?? '#333333',
                $data['visualIdentity']['colors']['secondaryText'] ?? '#666666',
                $data['visualIdentity']['colors']['button'] ?? '#e0e0e0',
                $data['visualIdentity']['colors']['buttonText'] ?? '#333333',
                ...
                $data['contactInformation']['email'] ?? null,
                $data['contactInformation']['phone'] ?? null,
                $data['contactInformation']['address'] ?? null
            ]);

            http_response_code(200);
            echo json_encode(['success' => true, 'message' => 'Changes saved successfully']);
        } catch(PDOException $e) {
            error_log("Failed to update branding data: " . $e->getMessage());
            http_response_code(500);
...
try {
    ...
    // Transform database data into the expected JSON structure
    $brandingConfig = [
        'companyInfo' => [
            'name' => $brandingData['company_name'] ?? '',
    ...
    error_log("Failed to fetch branding data: " . $e->getMessage());
    $brandingConfig = [
        'companyInfo' => ['name' => 'Company Name'],
        'visualIdentity' => [
            'logoUrl' => ['primary' => '', 'favicon' => '', 'socialShare' => ''],
            'colors' => [
                'background' => '#ffffff',
                'primaryText' => '#333333',
...
<body>
    ...
    <script>
        ...
        const brandingData = <?php echo json_encode($brandingConfig); ?>;
        
        // Load initial data
        populateForm(brandingData);
                ...
                for (const [key, value] of Object.entries(obj)) {
                    if (typeof value === 'object' && value !== null) {
                        ...
                        if (input) {
                            ...
                            // For color inputs, update the text input as well
                            if (input.type === 'color') {
